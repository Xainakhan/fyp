...
interface PriceRange {
  min: string; // store as string to avoid 0 sticking in input
  max: string;
}
...
const FindDoctorPage: React.FC = () => {
  ...
  // NOTE: start as empty strings so no default 0 appears
  ...
  // Helper for fee input: only digits, no leading zeros
  const sanitizeFeeInput = (value: string): string => {
    let v = value.replace(/[^\d]/g, ""); // keep digits only
    v = v.replace(/^0+(?=\d)/, ""); // remove leading zeros if more digits follow
    return v;
  };
  ...
  const searchDoctors = () => {
    ...
    // Convert string price range to numbers for filtering
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              <div className="flex items-center mb-2">
                <h4 className="text-xl font-bold text-gray-800 mr-3">
                  {doctor.name}
                </h4>
                {doctor.verified && (
          ...
          <div className="bg-green-50 p-6 rounded-2xl border border-green-200 mb-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {nearbyDoctors.slice(0, 4).map((doctor) => (
                <div
                  key={doctor.id}
                  className="bg-white p-4 rounded-xl border border-green-200 hover:shadow-md transition-all"
                >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {Object.keys(doctorDatabase).map((key) => {
            ...
            return (
              ...
              >
                <IconComponent
                  className="mx-auto mb-3 text-blue-600 group-hover:text-blue-700"
                  size={32}
                />
