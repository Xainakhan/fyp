// ─────────────────────────────────────────────────────────────────────────────
// SeverityChart.tsx
// Place this file in:  src/healthTimeline/components/SeverityChart.tsx
//
// Usage in HealthTimeline.tsx:
...
const StatCard: React.FC<StatCardProps> = ({ label, value, color, subtext }) => (
  <div style={whiteCard}>
    <p style={{ fontSize: 11, color: "#6b7280", margin: "0 0 3px", whiteSpace: "nowrap" }}>
      {label}
    </p>
    <p
      style={{
...
const BreakdownBar: React.FC<BreakdownItemType> = ({ severity, count, percentage }) => (
...
// ─────────────────────────────────────────────
// SeverityChart (main export)
// ─────────────────────────────────────────────

interface SeverityChartProps {
  /** Pass the symptoms array directly from HealthTimeline state */
...
const SeverityChart: React.FC<SeverityChartProps> = ({ symptoms }) => {
  ...
  // ── Chart.js render (re-runs only when data changes) ────────────────────
  useEffect(() => {
    ...
    const datasets: any[] = [
      {
        ...
        backgroundColor: visiblePoints.map((p) => `${SEVERITY_COLOR[p.severity]}cc`),
          ...
          tooltip: {
            callbacks: {
              ...
              label: (ctx) => {
                ...
                return v !== null ? ` Avg: ${v.toFixed(1)}` : null;
              },
            },
          },
        },
  ...
  const avgSub    = stats.avgValue > 0 ? (AVG_LABEL[Math.round(stats.avgValue)] ?? "Mixed") : undefined;
  ...
  return (
    ...
    >
      ...
      {/* Chart panel */}
      <div style={{ ...glassPanel, marginBottom: 12 }}>
